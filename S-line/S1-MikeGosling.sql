...
ALTER TABLE
    medewerkers
ADD
    COLUMN geslacht VARCHAR(1) CONSTRAINT m_geslacht_chk CHECK (geslacht IN ('M', 'V'));
...
CREATE SEQUENCE afdelingen_anr_seq INCREMENT 10 START 50 MINVALUE 50 MAXVALUE 120 CACHE 1;
...
ALTER COLUMN
    anr
SET
    DEFAULT NEXTVAL ('afdelingen_anr_seq');

INSERT INTO
    afdelingen (naam, locatie, hoofd)
VALUES
    ('MARKETING', 'ROTTERDAM', 7782);
...
INSERT INTO
    afdelingen (naam, locatie, hoofd)
VALUES
    ('HUMAN RESOURCES', 'UTRECHT', 8000);

INSERT INTO
    afdelingen (naam, locatie, hoofd)
VALUES
    ('COMMUNICATIE', 'ROTTERDAM', 7782);
...
CREATE TABLE adressen (
    pstcd VARCHAR(6) CONSTRAINT adr_pstcd_chk CHECK (pstcd ~* '[0-9]{4}[A-Z]{2}'),
    huisnmr VARCHAR(6) CONSTRAINT adr_huisnmr_nn NOT NULL,
    ingdatum DATE CONSTRAINT adr_ingd_nn NOT NULL,
    einddatum DATE CONSTRAINT adr_eindd_chk CHECK (einddatum >= ingdatum),
    telefnmr NUMERIC(10) CONSTRAINT adr_tlfnmr_u UNIQUE,
    med_mnr NUMERIC(4) CONSTRAINT m_mnr_fk REFERENCES medewerkers DEFERRABLE,
    PRIMARY KEY (pstcd, huisnmr, ingdatum)
);
